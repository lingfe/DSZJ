// pages/dripLove/rescueDetails/confirmRecord/confirmRecord.js
var app=getApp();
Page({
  ...
  onLoad: function (options) {
    ...
    //获取该项目的证实记录
    that.getConfirmRecord(that);
  },

  //获取该项目的证实记录
  getConfirmRecord: function (that) {
    //请求地址
    var url = app.config.dszjPath_web + "api/SeriousIllness/verifyPaging";
    //参数
    var data = {
      id: that.data.id,
    ...
    app.request.request(url, "get", header, data, function (res) {
      var pageList = that.data.list_zhengshi;
      //得到数据
